    ...
    if st.button("🔍 ANALIZUJ ZDJĘCIA"):
        if tmpl and imgs:
            with st.spinner("AI analizuje zdjęcia..."):
                b64_imgs = []
                for f in imgs:
                    f.seek(0) # Resetowanie pliku przed odczytem
                    b64_imgs.append(base64.b64encode(f.read()).decode('utf-8'))
                
                prompt = 'Jesteś asystentem biura nieruchomości. Przeanalizuj te zdjęcia i wyciągnij dane: Data, Wyposażenie, Energa, Klucze, Uwagi. Zwróć WYŁĄCZNIE JSON: {"Data":"","Wyposażenie":"","Energa":"","Klucze":"","Uwagi":""}'
                ...
                try:
                    ...
                    # BEZPIECZNIK: Sprawdzenie czy odpowiedź nie jest pusta
                    raw_content = res.choices[0].message.content
                    if raw_content:
                        st.session_state.extracted = json.loads(raw_content)
                        st.session_state.pdf_bytes = tmpl.read()
                        st.rerun()
                    else:
                        st.error("AI nie zwróciło tekstu. Spróbuj wgrać mniej zdjęć (max 5) lub lepszej jakości.")
                
                except Exception as e:
                    st.error(f"Błąd OpenAI: {str(e)}")

# 2. ROZMIESZCZANIE (Bez zmian)
if st.session_state.extracted:
    st.subheader("📍 KROK 2: Postaw tekst na dokumencie")
    st.info("Wybierz pole i dotknij palcem na dokumencie.")
    ...
    if st.session_state.active_key:
        st.warning(f"👉 Kliknij na dokumencie dla: {st.session_state.active_key}")

    if 'pdf_bytes' in st.session_state:
        img, pw, ph = get_preview(st.session_state.pdf_bytes)
        draw = ImageDraw.Draw(img)
        for k, p in st.session_state.placed.items():
            # Skrócony podgląd tekstu na mapie
            txt_preview = str(st.session_state.extracted[k])[:20] + "..."
            draw.text((p['x'], p['y']), txt_preview, fill="blue")
    ...
    # 3. PODPISY I FINALIZACJA (Bez zmian)
    st.divider()
